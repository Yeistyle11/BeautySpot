...
export default function ProfessionalActions({
  ...
  const handleToggleActive = () => {
    setConfirmDialog({
      title: isActive ? "Inactivar Profesional" : "Activar Profesional",
      message: isActive
        ? `¿Estás seguro de inactivar a ${professionalName}? No podrá recibir nuevas citas.`
        : `¿Estás seguro de activar a ${professionalName}?`,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (res.ok) {
            setToast({
              message: `Profesional ${isActive ? "inactivado" : "activado"} exitosamente`,
              type: "success",
            });
            ...
            try {
              ...
              setToast({
                message: data.error || "Error al actualizar el profesional",
                type: "error",
              });
            } catch {
              setToast({
  ...
  const handleDelete = () => {
    setConfirmDialog({
      ...
      message: `¿Estás seguro de eliminar a ${professionalName}? Esta acción no se puede deshacer. El profesional no debe tener citas pendientes o confirmadas.`,
        ...
        try {
          ...
          if (res.ok) {
            ...
            try {
              const data = await res.json();
              setToast({
                message: data.error || "Error al eliminar el profesional",
                type: "error",
              });
            } catch {
              setToast({
