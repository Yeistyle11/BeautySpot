...
describe('BookingEventListeners', () => {
  ...
  describe('handleUserRegistered', () => {
    it('debería loggear usuario registrado como CLIENT', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`Usuario registrado: ${event.payload.email}`);
      expect(logSpy).toHaveBeenCalledWith(`Cliente registrado en Booking Service: ${event.payload.email}`);
    });
  ...
  describe('handleBusinessCreated', () => {
    it('debería loggear negocio creado', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`Negocio creado: ${event.payload.businessId}`);
      expect(logSpy).toHaveBeenCalledWith(`Negocio ${event.payload.businessId} creado en Booking Service`);
    });
  });
  ...
  describe('handleProfessionalCreated', () => {
    it('debería crear disponibilidad semanal para profesional', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`Profesional creado: ${event.payload.professionalId}`);
      ...
      expect(logSpy).toHaveBeenCalledWith(`Disponibilidad semanal creada para profesional prof-123`);
    });
  ...
  describe('handlePaymentRegistered', () => {
    ...
    it('debería loggear pago sin cita', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`Pago registrado: ${event.payload.paymentId}`);
      expect(logSpy).not.toHaveBeenCalledWith(expect.stringContaining('vinculado a cita'));
    });
  });
  ...
  describe('handleAppointmentReminderDue', () => {
    it('debería loggear recordatorio programado', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`Recordatorio de cita pendiente: ${event.payload.appointmentId}`);
      expect(logSpy).toHaveBeenCalledWith(
