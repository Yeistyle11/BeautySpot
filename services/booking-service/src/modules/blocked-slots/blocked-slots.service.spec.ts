...
describe('BlockedSlotsService', () => {
  ...
  describe('findByProfessional', () => {
    ...
    it('debería retornar todos los blocked slots cuando futureOnly es false', async () => {
      ...
      const result = await service.findByProfessional('business-123', 'prof-123', false);
  ...
  describe('create', () => {
    ...
    it('debería propagar errores del repositorio', async () => {
      ...
      await expect(service.create('business-123', 'prof-123', data)).rejects.toThrow('Database error');
    });
  });
  ...
  describe('remove', () => {
    ...
    it('debería lanzar NotFoundException si el blocked slot no existe', async () => {
      ...
      await expect(service.remove('non-existent', 'business-123')).rejects.toThrow(NotFoundException);
      await expect(service.remove('non-existent', 'business-123')).rejects.toThrow('Bloqueo no encontrado');
    });

    it('debería propagar errores del repositorio', async () => {
      mockRepo.delete.mockRejectedValue(new Error('Database error'));

      await expect(service.remove('block-123', 'business-123')).rejects.toThrow('Database error');
    });
  });
});