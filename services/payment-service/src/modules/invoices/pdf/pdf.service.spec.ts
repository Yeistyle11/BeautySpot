...
describe('PdfService', () => {
  ...
  beforeEach(async () => {
    ...
    const PDFKitMock = jest.fn().mockReturnValue(mockDoc);
    jest.doMock('pdfkit', () => ({
      __esModule: true,
      default: PDFKitMock,
    }));
    ...
    mockConfigService = {
      get: jest.fn((key: string, defaultValue?: string) => {
        if (key === 'PDF_STORAGE_PATH') return defaultValue || './temp/invoices';
        return undefined;
      }),
    } as any;
  ...
  describe('generateInvoicePdf', () => {
    ...
    it('debería manejar factura sin logo', async () => {
      const invoiceData: InvoiceData = {
        ...
        invoiceDate: new Date('2023-11-16'),
        ...
        business: {
          ...
          email: 'info@stylestudio.co',
        },
        client: {
          name: 'María González',
          document: '987654321',
        },
        items: [{ name: 'Manicura', quantity: 1, price: 20000 }],
        subtotal: 20000,
        tax: 3800,
        total: 23800,
      ...
      await service.generateInvoicePdf(invoiceData);

      const pdfkitInstance = new (pdfkit.default)();
      expect(pdfkitInstance.fontSize).toHaveBeenCalled();
    });
  });
  ...
  describe('savePdfToBuffer', () => {
    ...
    it('debería crear directorio si no existe', async () => {
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith('./temp/invoices', { recursive: true });
    ...
    it('debería usar ruta personalizada del config', async () => {
      ...
      expect(mockConfigService.get).toHaveBeenCalledWith('PDF_STORAGE_PATH', './temp/invoices');
      expect(fs.writeFileSync).toHaveBeenCalledWith(
