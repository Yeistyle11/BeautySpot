...
describe('InvoicesService', () => {
  ...
  describe('findByBusiness', () => {
    ...
    it('debería filtrar por estado', async () => {
      ...
      const result = await service.findByBusiness('business-123', { status: InvoiceStatus.PAID });
  ...
  describe('findById', () => {
    ...
    it('debería lanzar NotFoundException si la factura no existe', async () => {
      ...
      await expect(service.findById('non-existent', 'business-123')).rejects.toThrow(NotFoundException);
      await expect(service.findById('non-existent', 'business-123')).rejects.toThrow('Factura no encontrada');
    });
  });
  ...
  describe('updateStatus', () => {
    it('debería actualizar el estado de la factura', async () => {
      ...
      const result = await service.updateStatus('invoice-123', 'business-123', InvoiceStatus.PAID);
  ...
  describe('generateInvoicePdf', () => {
    it('debería generar PDF de factura', async () => {
      mockInvoiceRepo.findOne.mockResolvedValue(mockInvoice);
      mockPdfService.generateInvoicePdf.mockResolvedValue(Buffer.from('PDF data'));

      const result = await service.generateInvoicePdf('invoice-123', 'business-123');
    ...
    it('debería lanzar NotFoundException si la factura no existe', async () => {
      ...
      await expect(service.generateInvoicePdf('non-existent', 'business-123')).rejects.toThrow(NotFoundException);
    });
  });
});