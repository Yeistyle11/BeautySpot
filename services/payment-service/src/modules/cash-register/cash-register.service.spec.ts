...
describe('CashRegisterService', () => {
  ...
  describe('closeSession', () => {
    it('debería cerrar una sesión exitosamente', async () => {
      ...
      await service.closeSession('session-123', 'business-123', 'user-123', dto);
    ...
    it('debería lanzar NotFoundException si la sesión no existe', async () => {
      ...
      await expect(
        service.closeSession('non-existent', 'business-123', 'user-123', { closingAmount: 50000 })
    ...
    it('debería lanzar BadRequestException si la sesión ya está cerrada', async () => {
      ...
      await expect(
        service.closeSession('session-123', 'business-123', 'user-123', { closingAmount: 50000 })
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.closeSession('session-123', 'business-123', 'user-123', { closingAmount: 50000 })
  ...
  describe('registerMovement', () => {
    it('debería registrar un movimiento exitosamente', async () => {
      ...
      const result = await service.registerMovement('session-123', 'business-123', 'user-123', dto);
    ...
    it('debería lanzar BadRequestException si la sesión está cerrada', async () => {
      ...
      ).rejects.toThrow('No se pueden registrar movimientos en una sesión cerrada');
    });
  });
    ...
    it('debería retornar resumen de la sesión', async () => {
      const sessionWithMovements = {
        ...
        movements: [
          ...
          { ...mockMovement, type: CashMovementType.OUT, amount: 5000, id: 'movement-456' },
        ],
        isOpen: true,
        initOpenedAt: () => {},
        generateId: () => {},
      ...
      const result = await service.getSessionSummary('session-123', 'business-123');
