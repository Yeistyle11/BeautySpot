...
export class SearchService {
  ...
  private async searchBusinesses(filters: SearchFilters): Promise<Omit<SearchResult, "type">> {
    ...
    if (filters.city) {
      qb.andWhere("bp.city ILIKE :city", { city: `%${escapeLikePattern(filters.city)}%` });
    }

    if (filters.businessType) {
      qb.andWhere("bp.business_type = :businessType", { businessType: filters.businessType });
    }
  ...
  private async searchProfessionals(filters: SearchFilters): Promise<Omit<SearchResult, "type">> {
    const page = filters.page || 1;
    const limit = Math.min(filters.limit || 20, 50);
    const offset = (page - 1) * limit;

    const qb = this.proRepo.createQueryBuilder("pp")
    ...
    if (filters.city) {
      qb.innerJoin("business_profiles", "bp", "bp.business_id = pp.business_id")
        .andWhere("bp.city ILIKE :city", { city: `%${escapeLikePattern(filters.city)}%` });
    }
