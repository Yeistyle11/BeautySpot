...
describe("ProfessionalProfilesService", () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<ProfessionalProfilesService>(
      ProfessionalProfilesService
    );
  });
  ...
  describe("updateProfile", () => {
    ...
    it("debería lanzar NotFoundException si el perfil no existe", async () => {
      ...
      await expect(
        service.updateProfile("non-existent", "business-123", {})
      ).rejects.toThrow(NotFoundException);
      await expect(
        service.updateProfile("non-existent", "business-123", {})
      ).rejects.toThrow("Perfil de profesional no encontrado");
    });
  });
  ...
  describe("toggleVisibility", () => {
    ...
    it("debería lanzar NotFoundException si el perfil no existe", async () => {
      ...
      await expect(
        service.toggleVisibility("non-existent", "business-123", true)
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("findVisibleByBusiness", () => {
    it("debería retornar profesionales visibles del negocio", async () => {
      const professionals = [
        mockProfessionalProfile,
        { ...mockProfessionalProfile, professionalId: "prof-456" } as any,
      ];
  ...
  describe("findTopRated", () => {
    it("debería retornar profesionales mejor calificados", async () => {
      const topRated = [
        mockProfessionalProfile,
        { ...mockProfessionalProfile, rating: 4.9 } as any,
      ];
